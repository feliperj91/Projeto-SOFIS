document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function generateClientReport(clientId) {
        ...
        // Header
        doc.setFillColor(44, 57, 63); // --bg-color
        doc.rect(0, 0, 210, 40, 'F');

        doc.setTextColor(255, 160, 0); // --accent
        doc.setFontSize(22);
        doc.setFont("helvetica", "bold");
        doc.text("SOFIS - Relatório de Acessos", 15, 25);

        doc.setTextColor(236, 239, 241); // --text-primary
        doc.setFontSize(10);
        doc.setFont("helvetica", "normal");
        doc.text(`Gerado em: ${new Date().toLocaleString('pt-BR')}`, 15, 33);

        // Client Name
        doc.setTextColor(44, 57, 63);
        doc.setFontSize(18);
        doc.setFont("helvetica", "bold");
        ...
        if (client.contacts && client.contacts.length > 0) {
            doc.setFontSize(14);
            doc.text("Contatos", 15, currentY);
            currentY += 5;

            const contactRows = client.contacts.map(con => [
                con.name || '-',
                (con.phones || []).join(', ') || '-',
                (con.emails || []).join(', ') || '-'
            ]);
        ...
        if (client.servers && client.servers.length > 0) {
            ...
            doc.setFontSize(14);
            doc.text("Servidores SQL", 15, currentY);
            currentY += 5;

            const serverRows = [];
            client.servers.forEach(s => {
                s.credentials.forEach((cred, idx) => {
                    serverRows.push([
                        idx === 0 ? s.environment : '',
                        idx === 0 ? s.sqlServer : '',
                        cred.label || '-',
            ...
            doc.autoTable({
                startY: currentY,
                head: [['Ambiente', 'Servidor', 'Tipo', 'Usuário', 'Senha']],
                body: serverRows,
                theme: 'grid',
                headStyles: { fillColor: [44, 57, 63] },
                margin: { left: 15 }
            });
            currentY = doc.lastAutoTable.finalY + 15;
        }
        ...
        if (client.vpns && client.vpns.length > 0) {
            if (currentY > 250) { doc.addPage(); currentY = 20; }
            doc.setFontSize(14);
            doc.text("VPN", 15, currentY);
            currentY += 5;

            const vpnRows = client.vpns.map(v => [
                v.user || '-',
                v.password || '-',
                v.notes || '-'
            ]);
            ...
            doc.autoTable({
                startY: currentY,
                head: [['Usuário', 'Senha', 'Observações']],
                body: vpnRows,
                theme: 'striped',
                headStyles: { fillColor: [45, 90, 110] },
                margin: { left: 15 }
            });
            currentY = doc.lastAutoTable.finalY + 15;
        }
        ...
        // URLs Section
        const urlCount = (client.urls ? client.urls.length : 0);
        if (urlCount > 0 || client.webLaudo) {
            if (currentY > 250) { doc.addPage(); currentY = 20; }
            doc.setFontSize(14);
            doc.text("Sistemas e URLs", 15, currentY);
            currentY += 5;

            const urlRows = (client.urls || []).map(u => [
                u.environment || '-',
                u.system || '-',
                u.bridgeDataAccess || '-',
                u.bootstrap || '-',
                u.notes || u.execUpdate || '-'
            ]);

            if (client.webLaudo) {
                urlRows.push(['Produção', 'WebLaudo', '-', '-', client.webLaudo]);
            }

            doc.autoTable({
                startY: currentY,
                head: [['Ambiente', 'Sistema', 'Bridge', 'Bootstrap', 'URL/Notas']],
                body: urlRows,
                theme: 'striped',
                headStyles: { fillColor: [66, 84, 93] },
                margin: { left: 15 }
            });
            currentY = doc.lastAutoTable.finalY + 15;
        }
        ...
        for (let i = 1; i <= pageCount; i++) {
            ...
            doc.text(`Página ${i} de ${pageCount} - Gerado por Projeto SOFIS`, 105, 290, { align: 'center' });
        }

        doc.save(`Relatorio_Acessos_${client.name.replace(/\s+/g, '_')}.pdf`);
        showToast(`✅ Relatório de "${client.name}" gerado com sucesso!`, 'success');
    }
            ...
            <div class="client-row-header" onclick="toggleClientRow('${client.id}')">
                ...
                <div class="header-right">
                     ...
                     <div class="row-actions">
                        ...
                        <button class="btn-icon" style="color: #4db6ac;" onclick="generateClientReport('${client.id}'); event.stopPropagation();" title="Gerar Relatório PDF">
